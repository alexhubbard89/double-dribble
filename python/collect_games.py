...
for year in range(2018, 2003, -1):
    ...
    while conintue_collecting == True:
        ...
        x = pd.read_sql_query("select * from ncaa_game_info", open_connection())
        teams_df = pd.read_sql_query("select * from ncaa_teams", open_connection())
        teams_df['team_id'] = teams_df['team_id'].astype(int)
        team_id_list = list(x['team_id'].drop_duplicates())
        teams_df = teams_df.loc[~teams_df['team_id'].isin(team_id_list)].reset_index(drop=True)

        print 'collecting more'
        print 'left to collect: {}'.format(len(teams_df))

        if len(teams_df) > 0:
            conintue_collecting = True
            i = np.random.choice(list(teams_df.index), 1)[0]
